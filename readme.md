
# Hackerakademi.dk/challenge - CTF writeup 
- Walk through of Forsvaretsefterretningstjeneste /challenge 2026 Akademi (with files)

## Writeup/Walk-through
- [Writeup/Walk-through (web)](https://hack-fe-2026.ckhansen.dk)
- [Writeup/Walk-through (markdown)](./readme_writeup_challenge.md)

## FE hacker-akademi/challenge:
- [hackerakademi.dk/challenge](https://hackerakademi.dk/challenge)

## Offentligtgørelse? 
- [FEs readme](./readme_fe_hackerchallenge.md)
```
FE skriver 

"Vi vil opfordre dig til at tage hensyn til andre operatører; uopfordrede 
spoilers kan opleves som ærgerlige. Når deadline er overstået, 
vil vi dog blive glade for at se offentlige writeups."
```

## Questions?
- hack-fe-2026@ckhansen.dk